...
<div class="@container flex h-full w-full min-w-0 flex-col overflow-hidden p-3 sm:p-4">
	...
	<div class="mt-1 min-h-0 flex-1">
		<EvilRadarChart {data} {config} class="h-full w-full">
			<EvilRadarChart.PolarGrid variant="circle" />
			<EvilRadarChart.PolarAngleAxis dataKey="capability" />
			<EvilRadarChart.Tooltip />
			<EvilRadarChart.Radar
				dataKey="target"
				variant="lines"
				radarProps={{ strokeDasharray: '4 4' }}
			/>
			<EvilRadarChart.Radar dataKey="current" variant="filled" isGlowing>
