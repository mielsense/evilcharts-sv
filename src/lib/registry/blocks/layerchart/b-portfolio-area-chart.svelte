...
<section
	class="@container/block flex size-full min-h-0 flex-col overflow-hidden pt-3 sm:pt-4"
	data-block="portfolio-area-chart"
>
	<div class="grid shrink-0 grid-cols-2 gap-x-5 px-3 sm:gap-x-8 sm:px-4">
		{#each series as item (item.key)}
			<div class="flex min-w-0 flex-col gap-0.5 @sm/block:gap-1">
				<div
					class="flex items-center gap-1.5 truncate text-[11px] text-muted-foreground @sm/block:text-sm"
				>
					<span class="size-2.5 shrink-0 rounded-full border-2" style:border-color={item.color}
					></span>
					{item.label}
				</div>
				<div class="text-xl font-semibold tracking-tight @sm/block:text-3xl">
	...
	<div class="mt-2 min-h-0 flex-1 overflow-hidden @sm/block:mt-4">
		<EvilAreaChart {data} {config} xDataKey="date" class="size-full" curveType="step">
			<EvilAreaChart.Tooltip variant="frosted-glass" />
			<EvilAreaChart.Area dataKey="robinhood" variant="dotted" strokeVariant="solid">
				<EvilAreaChart.ActiveDot variant="colored-border" />
			</EvilAreaChart.Area>
			<EvilAreaChart.Area dataKey="coinbase" variant="dotted" strokeVariant="solid">
				<EvilAreaChart.ActiveDot variant="colored-border" />
			</EvilAreaChart.Area>
		</EvilAreaChart>
	</div>
