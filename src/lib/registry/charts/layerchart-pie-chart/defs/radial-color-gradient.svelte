...
{#each sectors as { sectorKey, colorsCount } (sectorKey)}
	<linearGradient id={`${id}-colors-${sectorKey}`} x1="0" y1="0" x2="1" y2="1">
		{#if colorsCount === 1}
			<stop offset="0%" stop-color={chartColorVariable(sectorKey, 0)} />
			<stop offset="100%" stop-color={chartColorVariable(sectorKey, 0)} />
		{:else}
			{#each Array.from({ length: colorsCount }, (_, index) => `${(index / (colorsCount - 1)) * 100}%`) as offset, index (offset)}
				<stop {offset} stop-color={chartColorVariable(sectorKey, index, 0)} />
			{/each}
		{/if}
	</linearGradient>
{/each}
